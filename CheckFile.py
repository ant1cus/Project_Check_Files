...
from PyQt5.QtWidgets import QMainWindow, QApplication, QFileDialog, QMessageBox, QLineEdit, QDialog, QWidget
from check_file import CheckCS
from check_pemi import CheckPEMI
from start_checked import checked_pemi, checked_cs
...
class MainWindow(QMainWindow, Main.Ui_MainWindow):  # Главное окно
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pushButton_open_folder_cs.clicked.connect((lambda: self.browse(self.lineEdit_path_folder_cs)))
        self.pushButton_start_pemi.clicked.connect(self.check_pemi)
        self.pushButton_start_cs.clicked.connect(self.check_cs)
        ...
        self.thread_dict = {'check_cs': {}, 'check_pemi': {}}
        # Имена в файле
        self.name_list = {'lineEdit_path_pemi': ['Путь к папке ПЭМИ', self.lineEdit_path_pemi],
                          ...
                          'lineEdit_path_folder_cs': ['Путь к папке сплошного спектра', self.lineEdit_path_folder_cs]
                          }
        try:
            ...
            with open(pathlib.Path(pathlib.Path.cwd(), 'Настройки.txt'), "w", encoding='utf-8-sig') as f:
                data_insert = {"widget_settings": {"lineEdit_path_pemi": "", "checkBox_group_check": "",
                                                   ...
                                                   "lineEdit_path_folder_cs": ""},
                               "gui_settings":
                                   {"tab_order": {'0': "tab_pemi", '1': "tab_cs"}}
                               }
        ...
        self.tabWidget.setCurrentWidget(self.tabWidget.findChild(QWidget, self.tab_order['0']))
    ...
    def check_cs(self) -> None:
        file_name = self.logging_file('check_cs')
        try:
            self.logging_dict[file_name[0]].info('----------------Запускаем Check_cs----------------')
            self.logging_dict[file_name[0]].info('Проверка данных')
            folder = self.lineEdit_path_folder_cs.text().strip()
            continuous = checked_cs(folder)
            if isinstance(continuous, list):
                ...
                self.finished_thread('check_cs',
                                     name_all=str(pathlib.Path('logs', 'check_cs', file_name[1])),
                                     name_now=str(pathlib.Path('logs', 'check_cs', file_name[0])))
                return
            ...
            with open(pathlib.Path(pathlib.Path(self.default_path), 'Настройки.txt'), "w", encoding='utf-8-sig') as f:
                self.data["lineEdit_path_folder_cs"] = folder
                dict_load = {"widget_settings": self.data,
                             "gui_settings": {"tab_order": {'1': "tab_pemi", '0': "tab_cs"}}}
            ...
            continuous['move'] = len(self.thread_dict['check_cs'])
            self.thread = CheckCS(continuous)
            ...
            self.thread.start()
            self.thread_dict['check_cs'][str(self.thread)] = {'filename_all': file_name[1],
                                                              'filename_now': file_name[0]}
        except BaseException as exception:
            self.logging_dict[file_name[0]].error('Ошибка при старте check_cs')
    ...
    def check_pemi(self) -> None:
        ...
        try:
            ...
            with open(pathlib.Path(pathlib.Path(self.default_path), 'Настройки.txt'), "w", encoding='utf-8-sig') as f:
                ...
                dict_load = {"widget_settings": self.data,
                             "gui_settings": {"tab_order": {'0': "tab_pemi", '1': "tab_cs"}}}
                json.dump(dict_load, f, ensure_ascii=False, sort_keys=True, indent=4)
                self.data = dict_load["widget_settings"]
